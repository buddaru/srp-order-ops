...
export default function LocationApp() {
  ...
  // Base path for this location's routes
  const base = `/app/${locationSlug}`
  ...
  useEffect(() => {
    if (selectedStage !== 'picked-up' || pickedUpLoaded.current || !currentLocation) return
    ...
    <div className={`${styles.app} ${!sidebarOpen ? styles.sidebarCollapsed : ''} ${mobileSidebarOpen ? styles.sidebarOpen : ''}`}>
      ...
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.sidebarNav}>
          ...
          {navLink('orders',     <IconOrders />,   'Orders',         true)}
          {navLink('production', <IconProd />,     'Daily Production')}
          {navLink('waste',      <IconWaste />,    'Food Waste')}
          {navLink('schedule',   <IconSchedule />, 'Schedule')}
          ...
          {!sidebarOpen && <div style={{ height: 8 }} />}
          {navLink('settings', <IconSettings />, 'Settings')}
          {isAdmin && navLink('admin', <IconAdmin />, 'Team')}
          {isAdmin && navLink('menu',  <IconMenu />,  'Menu & Pricing')}
        </nav>
      ...
      <div className={styles.mainContent}>
        ...
        <Routes>
          ...
          {/* Fallback: redirect bare /app/:locationSlug to orders */}
          <Route index element={<RedirectToOrders base={base} />} />
        </Routes>
...
function RedirectToOrders({ base }) {
  const navigate = useNavigate()
  useEffect(() => { navigate(`${base}/orders`, { replace: true }) }, [])
  return null
}
